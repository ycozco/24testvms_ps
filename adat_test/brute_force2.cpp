...
#include <fstream>
#include <sstream>
#include <experimental/filesystem> // Cambiado
#include <chrono>
#include <iomanip>
#include <cstdlib>
#include <algorithm> // Para sort, max
#include <ctime>     // Para localtime, etc.

using namespace std;
namespace fs = std::experimental::filesystem; // Cambiado a experimental
...
pair<vector<int>, int> brute_force(const vector<vector<int>>& matriz) {
    ...
    return make_pair(mejor_secuencia, mejor_cmax);
}
...
int main() {
    ...
    for (size_t i = 0; i < archivos_procesados.size(); i++) {
        const string& archivo = archivos_procesados[i];
        ...
        // Llamada a la función Brute Force (sin structured bindings)
        auto resultado = brute_force(matriz);
        vector<int> mejor_secuencia = resultado.first;
        int mejor_cmax = resultado.second;
        ...
        for (size_t j = 0; j < mejor_secuencia.size(); j++) cout << mejor_secuencia[j] << " ";
        ...
        for (size_t j = 0; j < mejor_secuencia.size(); j++) salida_csv << mejor_secuencia[j] << " ";
        salida_csv << "\n";
    }
